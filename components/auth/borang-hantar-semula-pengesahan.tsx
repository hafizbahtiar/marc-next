...
export function BorangHantarSemulaPengesahan() {
  const [keadaan, action] = useActionState(hantarSemulaPengesahanAction, KEADAAN_AWAL);

  return (
    <form action={action} className="grid gap-4">
      <Notis ralat={keadaan.ralat} berjaya={keadaan.berjaya} />
      {/*
      ...
      */}
      <ButangHantar className="h-10 w-full rounded-md text-sm sm:w-auto sm:px-5">
